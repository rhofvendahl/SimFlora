...
function Geometry() {
  ...
  Geometry.stem = new THREE.CylinderGeometry(0.014, 0.04, 0.5, 3, 1, true );
  ...
  Geometry.branch = new THREE.CylinderGeometry(0.07, 0.1, 1, 5, 1, true );
  Geometry.branch.translate(0, 0.5, 0);
}
...
function MaplePart(parentPart, type) {
  ...
  if (type === "leaf") {
    ...
    this.minAngle = 20;
    this.maxAngle = 60;
    this.lengthFactor = 0.48;
    this.widthFactor = 0.33;
    ...
    this.minAngle = 20;
    this.maxAngle = 60;
    this.lengthFactor = Math.random()*.45 + 0.26;
    this.widthFactor = 0.26;
    ...
    this.minAngle = 90;
    this.maxAngle = 90;
    this.lengthFactor = Math.random() * 0.6 + 0.3;
...
MaplePart.prototype.update = function(time, lastTime) {
  var growthRate = Math.sin(Math.PI/8 + time.seasonRad) * 0.8 + 0.1;
