function RootPart(parentPart) {
  ...
  if (parentPart === undefined) {
    ...
    if (this.level < 4) {
      ...
      geometry = new THREE.CylinderGeometry(0.07, 0.1, 1, 3, 1, true );
      this.budGrowth = 7;
      this.numChildren = 2;
      this.minAngle = 0;
      this.maxAngle = 30;
      this.lengthFactor = 0.6* Math.random() + 0.1;
      ...
      this.minAngle = 10;
      this.maxAngle = 30;
      this.lengthFactor = 0.9*Math.random() + 0.1;
...
RootPart.prototype.update = function(time) {
  ...
  var growthRate = Math.sin(Math.PI/8 + time.seasonRad) + 0.1;
